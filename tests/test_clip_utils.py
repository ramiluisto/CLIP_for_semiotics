...
class TestClipUtils(unittest.TestCase):
    """
    Test cases for CLIP utilities.
    """

    @patch("src.clip_utils.CLIPModel")
    @patch("src.clip_utils.CLIPProcessor")
    def test_init(self, mock_processor, mock_model):
        ...
        self.assertIn(analyzer.device, ["cuda", "cpu"])

    def test_compare_image_with_texts(self):
    ...
    def test_analyze_folder(self, mock_listdir, mock_exists):
        ...
        with patch(
            ...
            result = clip_analyzer.analyze_folder(
        ...
        self.assertEqual(
            result[0]["filepath"], os.path.join("dummy_folder", "image1.jpg")
        )
        self.assertEqual(result[0]["similarities"], {"prompt1": 0.8, "prompt2": 0.6})

        self.assertEqual(
            result[1]["filepath"], os.path.join("dummy_folder", "image2.jpg")
        )
        self.assertEqual(result[1]["similarities"], {"prompt1": 0.7, "prompt2": 0.5})
        ...
        with patch(
            ...
            with self.assertRaises(ValueError):
                clip_analyzer.analyze_folder(
                    "non_existent_folder", ["prompt1", "prompt2"]
                )
